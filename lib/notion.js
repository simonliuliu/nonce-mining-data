...
function parseDollar(str) {
  ...
  const cleaned = str.replace(/[US$,\s]/g, "").trim();
...
//   单币现金成本   (rich_text)  → cash_cost_per_btc  (格式: "US$11,730.00")
//   单币能源成本   (rich_text)  → energy_cost_per_btc
...
async function getAllCompanyProfiles(locale) {
...
async function getCompanyProfile(ticker, locale) {
  const dbId = process.env.NOTION_PROFILES_DB;
  if (!dbId) return null;
  ...
  const tryFetch = async (lang) => {
    try {
      const filter = lang
        ...
        : { property: "Slug", rich_text: { equals: ticker.toUpperCase() } };
      const r = await notion.databases.query({ database_id: dbId, filter, page_size: 1 });

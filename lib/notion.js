...
const NAME_MAP = {
  ...
  "Cango Inc.":          "Cango",
  "Hut 8":               "Hut 8",
  "Hut":                 "Hut 8",   // Notion 里的历史简写，统一为 Hut 8（同一 ticker HUT）
  // Ticker 反查
...
// ─── 公司改名合并 ──────────────────────────────────────────────
// 同一家公司在不同季度换了名称/ticker（改名），需要在数据层合并成同一个实体
// （canonical company/ticker），这样列表、排名、趋势、对比、详情页才能贯通、
// 不再出现两条记录。列表里仍按当季的实际名称展示（display_company/display_ticker）。
//   Bitfarms (BITF)                       —— 2025Q4 及之前
//   Keel Infrastructure(Bitfarms) (Keel)  —— 2026Q1 起
const ENTITY_MERGE = {
  "Keel Infrastructure(Bitfarms)": { company: "Bitfarms", ticker: "BITF" },
};
...
async function _getQuarterlyData() {
  ...
  for (const page of pages) {
    ...
    // 改名合并：canonical 用于所有按公司/ticker 的查询（保证历史贯通、列表不重复）；
    // display_* 保留当季原始名称，仅用于列表展示。普通公司两者一致。
    const merge        = ENTITY_MERGE[company] || ENTITY_MERGE[rawName];
    const canonCompany = merge ? merge.company : company;
    const canonTicker  = merge ? merge.ticker  : ticker;

    const key = `${canonCompany}|${quarter}`;
